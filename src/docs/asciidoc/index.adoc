...
that describes the problem. `error` and `_links.index.href` are always contained in the response.

For example, a request that attempts to retrieve a non-existent account will produce a
`404 Not Found` response:

include::{snippets}/get-non-existent-account-detail/http-response.adoc[]
